...
$sql = "SELECT city,province FROM `seat` WHERE id = $seat_id and status=\"NA\";";
...
//male votes
$sql_male = "select count(Vote2)as m_votes from user where seat2=$seat_id and gender=\"M\";";
$result_M = mysqli_query($conn, $sql_male);
$row = mysqli_fetch_assoc($result_M);
$male= $row["m_votes"];
//female votes
$sql_female = "select count(Vote2)as f_votes from user where seat2=$seat_id and gender=\"F\";";
...
?>
  ...
  <table>
    ...
    <tr>
      <th>Name</th>
      <td colspan="4"><?php echo $city . ', ' . $province; ?></td>
    </tr>
      <tr>
    ...
    <?php
      $count=0;
      $sql_name = "select name from party where party_id in (select DISTINCT vote2 from user where seat2=$seat_id);";
      ...
      while($row_n= mysqli_fetch_assoc($result)) 
      {
          ...
          $sql_v2 = "select count(vote2) as num_v2 from user where Vote2 = (select party_id from party ".$condition.") and seat2=$seat_id;";
          $result_v2 = mysqli_query($conn, $sql_v2);
          $row_count = mysqli_fetch_assoc($result_v2);
          $vote_count= $row_count["num_v2"];
          $count++; ?>
